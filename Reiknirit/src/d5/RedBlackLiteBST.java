...
public class RedBlackLiteBST<Key extends Comparable<Key>, Value> {
    ...
    *************************************************************************/
    ...
    private int countRedNodes(Node node){
    	...
    	if(node.color == true){
    		return 1 + countRedNodes(node.left) + countRedNodes(node.right);
    	}
    	return countRedNodes(node.left) + countRedNodes(node.right);
    }
    ...
    *************************************************************************/
    public static void main(String[] args) {
    	int N = StdIn.readInt();
    	double RednodePercent = 0.0;
    	for(int i = 0; i < 100; i++){
    		RedBlackLiteBST<Integer, Integer> st = new RedBlackLiteBST<Integer, Integer>();
    		for(int j = 0; st.numberOfNodes() < N; j++){
    			st.put(StdRandom.uniform(2147483647), j); 
    		}
    		RednodePercent += (double ) st.countRedNodes() / N;
    	}
    	StdOut.println(RednodePercent/100);
    }
}